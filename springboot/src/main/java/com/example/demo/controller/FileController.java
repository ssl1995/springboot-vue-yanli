...
public class FileController extends BaseController {
    ...
    public Result<?> upload(MultipartFile file) throws IOException {
        ...
        return Result.success("http://" + ip + ":" + port + "/files/" + flag+ "_" + originalFilename);  // 返回结果 url
    }
    ...
    public JSON uploadvideo(MultipartFile file) throws IOException {
        String originalFilename = file.getOriginalFilename();  // 获取源文件的名称
        // 定义文件的唯一标识（前缀）
        String flag = IdUtil.fastSimpleUUID();
        String rootFilePath = System.getProperty("user.dir") + "/files/" + flag + "_" + originalFilename;  // 获取上传的路径
...
//            if (StrUtil.isNotEmpty(fileName)) {
                ...
                byte[] bytes = FileUtil.readBytes(basePath + flag);  // 通过文件的路径读取文件字节流
...
//            }
    ...
    public void previewFile(@PathVariable String fileName, HttpServletResponse response) throws IOException {
        showImg(System.getProperty("user.dir") + "/files/"+fileName, response);
    }
